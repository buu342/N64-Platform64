...
:CheckDebugMode
for /F "skip=1 delims=" %%i in (debug.h) do if not defined debugcheck set "debugcheck=%%i"
set "debugcheck=%debugcheck:~19,1%"
if "%debugcheck%"=="1" (
...
:MoveTo64Drive
cd out
for /R "%~dp0out" %%f in (*.n64) do copy /y "%%f" "%N64DRIVE%" >nul
if "%FORCEMOVE%"=="0" (
	echo Done! 
) else (
